...
interface ParentCancelRescheduleModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function ParentCancelRescheduleModal({
  isOpen,
  onClose,
  onCancel,
  onReschedule,
  classInfo,
}: ParentCancelRescheduleModalProps) {
  ...
  const cancelClassMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["parentStudentClassData"] });
      resetForm();
      onClose();
    },
    onError: (error: any) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue={classInfo?.canCancel ? "cancel" : "reschedule"} className="w-full">
          <TabsList className={`grid w-full ${classInfo?.canCancel && classInfo?.canReschedule ? 'grid-cols-2' : 'grid-cols-1'}`}>
            {classInfo?.canCancel && (
              <TabsTrigger value="cancel">Cancel Class</TabsTrigger>
            )}
            {classInfo?.canReschedule && (
          ...
          {classInfo?.canCancel && (
            ...
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                ...
                {classInfo?.start_date ? format(new Date(classInfo.start_date), "PPP") : "Date"}{" "}
                {classInfo?.start_time || "Time"}
              </div>

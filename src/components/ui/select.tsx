...
>(({ className, children, position = "popper", searchable, searchPlaceholder = "Search...", ...props }, ref) => {
      ...
      return React.Children.map(children, (child) => {
        if (React.isValidElement(child)) {
          ...
          if (child.props.children) {
            const filteredNestedChildren = filterChildren(child.props.children);
            if (React.Children.count(filteredNestedChildren) > 0) {
              return React.cloneElement(child, {}, filteredNestedChildren);
            }
            return null;
          }
        }
  ...
  }, [searchValue, children, searchable]);
          ...
          {searchable && (
            <div className="flex items-center px-2 pb-2">
              ...
              <Input
                ...
                className="h-8 w-full border-0 p-0 text-sm focus-visible:ring-0 focus-visible:ring-offset-0"
                onKeyDown={(e) => {
                  e.stopPropagation();
                  if (e.key === 'Escape') {
                    setSearchValue('');
